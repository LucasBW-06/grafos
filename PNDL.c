// Grafo ponderado não direcional por listas de adjacencias
...
bool inserir_aresta_aux(Grafo* g, int origem, int destino, float peso) {
...
bool inserir_aresta(Grafo* g, int origem, int destino, float peso) {
    ...
    if (inserir_aresta_aux(g, origem, destino, peso)) {
        inserir_aresta_aux(g, destino, origem, peso);
        g->numArestas++;
    }
    return true;
}
